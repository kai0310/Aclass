...
<div class=card>
  ...
  <div class="description border">
    <ul>
      <li>課題名：{{decryptData($submission->task['title'], 'DATA_KEY')}}</li>
      <li>課題送信者：{{decryptData($submission->user['name'], 'USER_KEY')}}</li>
      <li>提出日時：{{decryptData($submission->user['name'], 'USER_KEY')}}</li>
      <li><a class="button first-button" href="{{route('taskSingle', ['task_id' => $submission->task['id']])}}">課題を見る <i class="fas fa-angle-right"></i></a></li>
    </ul>
  </div>
